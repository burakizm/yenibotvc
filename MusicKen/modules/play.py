...
def updated_stats(chat, queue, vol=150):
    if chat.id in callsmusic.pytgcalls.active_calls:
        ...
        if len(que) > 0:
            ...
            stats += "Şarkı çalınıyor : **{}**\n".format(queue[0][0])
            stats += "isteyen : {}".format(queue[0][1].mention)
    else:
        stats = None
    return stats
...
async def ee(client, message):
    ...
    if stats:
        ...
        await message.reply("**lütfen önce botu açın!**")
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("**music çalmıyor**")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if (chet_id not in callsmusic.pytgcalls.active_calls) or (
            ...
            await cb.answer("sohbete bağlı değil!", show_alert=True)
        else:
        ...
        if (chet_id not in callsmusic.pytgcalls.active_calls) or (
            ...
            await cb.answer("sohbete bağlı değil veya zaten oynatılıyor", show_alert=True)
        else:
            callsmusic.pytgcalls.resume_stream(chet_id)
            await cb.answer("Music oynatılıyor!")
    elif type_ == "durdur":
        if (chet_id not in callsmusic.pytgcalls.active_calls) or (
            callsmusic.pytgcalls.active_calls[chet_id] == "paused"
        ):
            await cb.answer("sohbete bağlı değil veya zaten Duraklatıldı", show_alert=True)
        else:
            callsmusic.pytgcalls.pause_stream(chet_id)

            await cb.answer("Music Duraklatıldı!")
    elif type_ == "cls":
...
@Client.on_message(command(["ytplayer", "yt", "p"]) & other_filters)
@errors
    ...
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"❌ **dakikadan uzun videolar oynatılamaz** `{DURATION_LIMIT}` **menit tidak boleh diputar!**"
            )
        ...
        try:
            ...
            await lel.edit(
                "**Music bulunamadı.** Daha net şarkı sözleri ile arama yapmayı deneyin, Ketik `/help` bila butuh bantuan"
            )
            print(str(e))
            return
        dlurl = url
...
async def stream(_, message: Message):
    ...
    if audio:
        ...
        return await lel.edit_text("❗ lütfen bana çalabileceğim şarkı verin !")
